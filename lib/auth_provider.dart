...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> tryAutoLogin() async {
    ...
    try {
      ...
      if (lastLogin != null && lastLogin.isNotEmpty) {
        _user = await DatabaseService.getUserByLogin(lastLogin);
      }
    } catch (e) {
  ...
  Future<bool> login(String login, String password) async {
    ...
    notifyListeners();
    try {
      final user = await DatabaseService.getUserByLogin(login.trim());
      if (user == null) {
      ...
      _user = user;
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('last_user', login.trim().toLowerCase());
      _isLoading = false;
      notifyListeners();
      return true;
    } catch (e) {
